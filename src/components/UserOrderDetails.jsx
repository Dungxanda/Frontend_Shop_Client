...
const UserOrderDetails = () => {
  ...
  const cancelOrderHandler = async () => {
    await axios
      .put(`${server}/order/cancel-order/${id}`, {
        status: "Cancel",
      })
      .then((res) => {
        toast.success("Đơn hàng đã được hủy thành công!");
        dispatch(getAllOrdersOfUser(user._id)); // Tải lại danh sách đơn hàng sau khi hủy
      })
      .catch((error) => {
  ...
  return (
    <div className={`py-4 min-h-screen ${styles.section}`}>
      ...
      <div className="w-full flex items-center justify-between pt-6">
        ...
        <h3 className="text-[18px] font-bold text-red-500 text-center mt-4">
