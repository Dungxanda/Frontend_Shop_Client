  ...
  const vnpayPaymentHandler = async (e) => {
    ...
    try {
      ...
      const vnpayData = {
        ...
        orderId: orderData?.orderId, // Use unique order ID
        orderDescription: "Thanh toán đơn hàng #" + orderData?.orderId,
        // Add more fields if necessary
      };
  
      // Chỉ thêm /vnpay/create-payment-url nếu server không chứa sẵn /api/v2
      const { data } = await axios.post(`${server}/vnpay/create_payment_url`, vnpayData, config);
  
      if (data.paymentUrl) {
        // Redirect the user to VNPay's payment URL
      ...
      <div>
        ...
        {select === 4 && (
          <div className="w-full flex border-b">
            <div
              className={`${styles.button} !bg-[#f63b60] text-white h-[45px] rounded-[5px] cursor-pointer text-[18px] font-[600]`}
              onClick={vnpayPaymentHandler} // Call VNPay handler here
            >
              Thanh toán ngay
            </div>
          </div>
