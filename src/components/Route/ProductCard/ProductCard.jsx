...
const ProductCard = ({ data, isEvent }) => {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 relative">
      <Link
      ...
      {/* Kiểm tra nếu hết hàng thì hiển thị thông báo "Hết hàng" */}
      {data.stock === 0 ? (
        <div className="mt-2 text-red-500 font-semibold">Hết hàng</div>
      ) : (
        <div className="flex justify-between items-center mt-2">
          {click ? (
            <AiFillHeart
          ...
          <AiOutlineShoppingCart
            size={25}
            className={`cursor-pointer ${data.stock === 0 ? "text-gray-400 cursor-not-allowed" : ""}`}
